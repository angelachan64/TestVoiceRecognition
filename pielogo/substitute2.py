...
global List
def TurnToList():
       reader = csv.reader(file('/home/bamboo/Downloads/eliza.py-0.2/regular.csv', 'rb'))
       NewList=[]
       for item in reader:
              NewList.append(item)
       NewList=NewList[0]
...
#last replacement
              if (re.search(r'last\d* end',command)) or (re.search(r'\[last\d*',command)):
      
                     
                     CNumber=int(filter(str.isdigit,command.split("last")[1]))
                     ...
                     for n in (i-CNumber,i-1):
                            List[n]="del"
                     
                     
#follow replacement
              if re.search(r'follow\d* end',command):
                     
                     CNumber=int(filter(str.isdigit,command.split("follow")[1]))
                     Total=""
                    
                     for n in range(CNumber):
                             Total=Total+" "+List[i+n+1]
                            
                     List[i]=re.sub(r'follow\d*',Total,List[i])
                     
                     for n in (i+1,i+CNumber):
                            List[n]="del"
